...
def test_IN01_title_validation(driver):  #Caso para validar el titulo de la pagina
    login_page = LoginPage(driver)
    login_page.login_completo("standard_user","secret_sauce")
    titulo = driver.title                #.title obtiene el título de la pestaña del navegador y en este caso lo guarda en la variable "titulo"
    assert "Swag Labs" in titulo, "El título de la página es incorrecto." #verifica que "Swag Labs" esté contenido en el titulo

def test_IN02_prods_in_catalog(driver):  #Caso para validar que se vean productos
    login_page = LoginPage(driver)
    login_page.login_completo("standard_user","secret_sauce")
...
def test_IN03_ui_validation(driver):     #Caso para buscar diversos elementos en la UI y verificar que sean visibles
    login_page = LoginPage(driver)
    login_page.login_completo("standard_user","secret_sauce")
    inventory_page = InventoryPage(driver)
...
def test_IN04_product_validation(driver):    #Caso para validar que el nombre y precio del producto se visualice (tomando el primer producto que encuentre)
    login_page = LoginPage(driver)
    login_page.login_completo("standard_user","secret_sauce")
    inventory_page = InventoryPage(driver)
    
    inventory_page.obtener_productos() #Busca todos los elementos con la clase "inventory_item"
